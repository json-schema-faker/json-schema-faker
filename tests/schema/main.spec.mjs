...
const { only, all } = getTests(`${process.cwd()}/tests/schema`);
  ...
  describe(`${suite.description} (${relativeFilename})`, () => {
    suite.tests.forEach(test => {
      ...
      it(test.description, async () => {
        ...
        if (test.random) {
          jsf.option('random', seedrandom(test.random));
        } else {
          jsf.option({
        ...
        if (test.require) {
          ...
          await Promise.all(test.require.map(toRequire => import(`./${toRequire}.mjs`).then(mod => mod.register(jsf))));
        }
