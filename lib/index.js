    ...
    Object.defineProperty(OptionRegistry.prototype, "defaults", {
        get: function () {
            ...
            data['random'] = Math.random;
                         // ALISTER   
             data['arraySizeCallback'] = null;   
             data['schemaChoiceCallback'] = null;   
...
var arrayType = function arrayType(value, path, resolve, traverseCallback) {
    ...
    if (optionAPI('maxItems')) {
        ...
        if (minItems && minItems > optionAPI('maxItems')) {
         ...
         // ALISTER   
     if (optionAPI('arraySizeCallback')){   
         var requestedSize = optionAPI('arraySizeCallback')(path, minItems, maxItems);   
     
         if ((requestedSize >= minItems || minItems === undefined) && (requestedSize <= maxItems || maxItems === undefined)) {   
             minItems = requestedSize;   
             maxItems = requestedSize;   
         } else {   
             throw new Error('Requested size does not fit within bounds');   
         }   
     }   
    }
...
function traverse(schema, path, resolve, rootSchema) {
    ...
    if (typeof schema.thunk === 'function') {
        return traverse(schema.thunk(path), path, resolve);   
    }
        ...
        var result = traverse(schema, [], function reduce(sub, maxReduceDepth) {
            ...
            if (Array.isArray(sub.oneOf || sub.anyOf)) {
                ...
                return {
                         ...
                         if (optionAPI('schemaChoiceCallback')) {   
                             var requestedType = optionAPI('schemaChoiceCallback')(path, mix);   
                             utils.merge(copy, requestedType);   
                         }   
                         else {   
